...
static void map_cachable(uint64_t start, uint64_t size)
{
	map_region(start, start, size, PMD_SECT_AP_WRITE |
...
static int mmu_init(void)
{
	...
	if (get_sctlr() & CR_M) {
		...
		set_ttbr_tcr_mair(1, (uint64_t)ttb, TCR_FLAGS, MEMORY_ATTR);
	}
